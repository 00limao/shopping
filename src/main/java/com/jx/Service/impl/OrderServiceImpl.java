...
import com.alipay.deno.trade.DemoHbRunner;
import com.alipay.deno.trade.Main;
...
import com.jx.vo.CartOrderItemVO;
import com.jx.vo.OrderItemVO;
import com.jx.vo.OrderVO;
import com.jx.vo.ShippingVO;
...
public class OrderServiceImpl implements IOrderService {
    ...
    public ServerResponse createOrder(Integer userId, Integer shippingId) {
        ...
        //根据userId查询购物车中以选中的商品-->List<Cart>
        ...
        Order order = getOrder(userId, shippingId, orderTotalPrice);
        if(order==null){
    ...
    //创建订单
    private Order getOrder(Integer userId, Integer shippingId, BigDecimal orderTotalPrice){
        ...
        order.setPostage(0);
        order.setPaymentType(Const.PaymentEnum.ONLIEN.getCode());
        //保存订单
    ...
    private ServerResponse getCartOrderItem(Integer userId,List<Cart> cartList){
        ...
        for(Cart cart:cartList){
            ...
            if(product.getStatus()!= Const.ProductStatusEnum.PRODUCT_ONLINE.getCode()){
                return ServerResponse.createServerResponseByError("id为"+cart.getId()+"的商品已经下架");
            }
            if(product.getStock()<cart.getQuantity()){
                return ServerResponse.createServerResponseByError("id为"+cart.getProductId()+"的商品库存不足");
            }
    ...
    @Override
    public ServerResponse alipay(Integer userId, Long orderNo) {
        if(orderNo==null){
    ...
    public ServerResponse alipay_callback(Map<String, String> map) {
        ...
        Order order = orderMapper.findOrderByOrderNo(orderNo);
        if(order==null){
            System.out.println("订单"+orderNo+"不是本商品的订单");
        ...
        System.out.println("更新的行数："+result);
        if(result>0){
            System.out.println("支付信息保存成功");
            return ServerResponse.createServerResponseBySuccess(payInfo);
        }
        System.out.println("支付信息保存失败");
    ...
    public ServerResponse pay(Order order) throws IOException {
        ...
        AlipayTradePrecreateRequestBuilder builder = new AlipayTradePrecreateRequestBuilder()
                ...
                .setNotifyUrl("http://39.106.5.54:8080/order/alipay_callback.do")//支付宝服务器主动通知商户服务器里指定的页面http路径,根据需要设置
        ...
        switch (result.getTradeStatus()) {
            case SUCCESS:
                ...
                // 需要修改为运行机器上的路径
                String filePath = String.format("/ftpfile/qrcode/qr-%s.png",
